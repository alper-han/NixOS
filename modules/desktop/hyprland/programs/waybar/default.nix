    ...
    (_: {
      programs.waybar = {
        ...
        settings = [{
          ...
          modules-left = [ "custom/icon" "hyprland/workspaces" "cava" ];
          ...
          "custom/powerprofile" = {
            exec = "${../../scripts/PowerStatus.sh}";
            interval = 5;
            on-click = "${../../scripts/TogglePowerMode.sh}";
            return-type = "json";
            format = "{}";
          ...
          "custom/hyprsunset" = {
            exec = ''
              sh -c 'pgrep -x hyprsunset >/dev/null && echo "🌙" || echo "☀️"'
            '';
            interval = 2;
            on-click = ''
              sh -c 'pgrep -x hyprsunset >/dev/null && pkill hyprsunset || ${pkgs.hyprsunset}/bin/hyprsunset --temperature 3500 &'
            '';
          };
